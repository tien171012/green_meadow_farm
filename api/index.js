...
/* =========================== RÚT TIỀN ===========================
   Quy đổi mặc định: 100 vàng = 1đ.
   Có thể chỉnh 2 hằng số này nếu muốn đổi tỷ lệ/minimum.
   Tiền được admin chuyển thủ công qua ZaloPay/MoMo/ngân hàng sau khi duyệt.
================================================================== */
const WITHDRAW_GOLD_PER_VND = 100;
const MIN_WITHDRAW_VND = 1000;
const MAX_WITHDRAW_VND = 5000000;
...
const WEEKLY_PRIZE_POOL_PERCENT = 0.7; // 70% doanh thu ZaloPay THẬT trong tuần mới đem chia giải — không bao giờ lỗ
const MIN_WEEKLY_SPEND_TO_QUALIFY = 1000; // phải nạp tối thiểu 1.000đ qua ZaloPay TRONG TUẦN mới được vào bảng xếp hạng/nhận giải

// Tuần ĐẦU TIÊN game hoạt động: giải thưởng CỐ ĐỊNH (dù doanh thu bao nhiêu), để tạo động lực ban đầu.
// Từ tuần thứ 2 trở đi, TỰ ĐỘNG chuyển sang tính theo % doanh thu (an toàn, không bao giờ lỗ).
const FIRST_WEEK_START = Date.UTC(2026, 7, 10, 0, 0, 0); // Thứ Hai 10/08/2026 — sửa lại nếu deploy trễ hơn
const FIRST_WEEK_FIXED_PRIZES = [12000, 10000, 8000]; // Top1, Top2, Top3 — cố định riêng tuần đầu
...
// Tổng số tiền mỗi người chơi đã NẠP THẬT (đã được admin duyệt) trong tuần — dùng để chặn
// người cày miễn phí hoàn toàn (chưa từng nạp) vào top ăn giải thưởng tiền thật.
async function getWeeklySpendMap(weekStart) {
  const weekEnd = weekStart + 7 * 24 * 60 * 60 * 1000;
  const rows = await PaymentRequest.aggregate([
    { $match: { status: 'approved', createdAt: { $gte: new Date(weekStart), $lt: new Date(weekEnd) } } },
    { $group: { _id: '$telegramId', total: { $sum: '$price' } } },
  ]);
...
/* =========================== MỜI BẠN BÈ — HOA HỒNG 10% (15% nếu đã từng nạp) =========================== */
...
// Người mời ĐÃ TỪNG NẠP (bất kỳ lúc nào, không cần đúng tuần này) được tăng lên 15%.
const REFERRAL_COMMISSION_RATE = 0.10;
const REFERRAL_COMMISSION_RATE_BOOSTED = 0.15;
...
const COMBO_PACKAGES = {
  starter: { price: 1000, goldReward: 500000, oneTimeOnly: true, name: 'Gói khởi đầu', desc: 'Ưu đãi CHỈ 1 LẦN cho người chơi mới — nhận ngay 500.000 vàng' },
...
const PlayerSchema = new mongoose.Schema({
  ...
  comboFedUntil: { type: Number, default: 0 },                     // timestamp (ms) — còn hạn thì TẤT CẢ con vật không đói
  comboGoldBonusUntil: { type: Number, default: 0 },                // timestamp (ms) — còn hạn thì +30% giá trứng mọi con (combo1)
  hasEverPaid: { type: Boolean, default: false },                    // đã từng nạp qua ZaloPay ít nhất 1 lần chưa (bất kỳ lúc nào)
  starterPackUsed: { type: Boolean, default: false },                 // đã mua Gói khởi đầu chưa (chỉ mua được 1 lần)
}, { timestamps: true });
...
// Yêu cầu rút tiền: vàng bị trừ ngay khi tạo yêu cầu, nếu admin từ chối thì hoàn lại vàng.
...
function settleEggs(player) {
  ...
  for (const animal of ANIMALS) {
    ...
    const hasComboFed = player.comboFedUntil > now; // combo bảo vệ TẤT CẢ con vật, không cần chọn từng con
    const isFed = hasComboFed || (fedUntilTs !== undefined && fedUntilTs > now);
...
function effectiveEggValue(player, animal) {
  ...
  if (player.comboGoldBonusUntil > Date.now()) value = Math.round(value * 1.3); // combo1: +30% mọi con
  return value;
}
...
async function payReferralCommission(player, goldGain) {
  ...
  if (!referrer) return;
  const rate = referrer.hasEverPaid ? REFERRAL_COMMISSION_RATE_BOOSTED : REFERRAL_COMMISSION_RATE;
  const commission = Math.round(goldGain * rate);
...
function serializeState(player) {
  ...
  return {
    ...
    comboFedUntil: player.comboFedUntil,
    comboGoldBonusUntil: player.comboGoldBonusUntil,
    feedGoldPrice: FEED_GOLD_PRICE,
    freeFoodTokens: player.freeFoodTokens,
    referralCount: player.referralCount,
    referralCommissionRate: player.hasEverPaid ? REFERRAL_COMMISSION_RATE_BOOSTED : REFERRAL_COMMISSION_RATE,
    hasEverPaid: player.hasEverPaid,
    starterPackUsed: player.starterPackUsed,
...
app.get('/api/leaderboard', asyncHandler(async (req, res) => {
  const ws = getCurrentWeekStart();
  const isFirstWeek = ws === FIRST_WEEK_START;
  let prizes, pool;
  if (isFirstWeek) {
    prizes = FIRST_WEEK_FIXED_PRIZES;
    pool = prizes.reduce((a, b) => a + b, 0); // chỉ để hiển thị, không dùng để giới hạn tuần đầu
  } else {
    pool = await getWeeklyPrizePool(ws);
    prizes = distributePrizePool(pool);
  }
  ...
  const candidates = await Player.find({}).sort({ weeklyGold: -1 }).limit(200)
    .select('username telegramId weeklyGold weeklyGoldWeekStart level -_id').lean();
  const spendMap = await getWeeklySpendMap(ws);
  ...
  const ranked = candidates
    .map(p => ({
      ...
      weeklySpend: spendMap.get(p.telegramId) || 0,
    }))
    .filter(p => p.weeklyGold >= WEEKLY_PRIZE_MIN_GOLD) // chỉ tính người đạt tối thiểu mới lên bảng/nhận giải
    .filter(p => p.weeklySpend >= MIN_WEEKLY_SPEND_TO_QUALIFY) // phải nạp thật tối thiểu mới được nhận giải
  ...
  res.json({
    ...
    minSpendToQualify: MIN_WEEKLY_SPEND_TO_QUALIFY,
  });
}));
...
// Mua gói combo — áp dụng cho TẤT CẢ con vật, không cần chọn từng con. Cùng luồng ZaloPay + admin duyệt.
app.post('/api/request-combo', authMiddleware, asyncHandler(async (req, res) => {
  const { comboId } = req.body || {};
  const combo = COMBO_PACKAGES[comboId];
  if (!combo) return res.status(400).json({ error: 'Gói combo không hợp lệ.' });

  const player = req.player;
  if (combo.oneTimeOnly && player.starterPackUsed) {
    return res.status(400).json({ error: 'Bạn đã mua Gói khởi đầu rồi, mỗi người chỉ mua được 1 lần.' });
  }
  ...
  if (ADMIN_TELEGRAM_ID) {
    await sendTelegramMessage(ADMIN_TELEGRAM_ID,
      `🔔 Yêu cầu mua combo mới\n` +
      `Người chơi: ${player.username || 'ẩn danh'} (id: ${player.telegramId})\n` +
      `Gói: ${combo.name}\n` +
      `Số tiền cần nhận: ${combo.price.toLocaleString('vi-VN')}đ\n\n` +
      `Sau khi kiểm tra đã nhận đủ tiền trên ZaloPay, gõ:\n` +
      `/approve ${request._id}\n` +
      `(hoặc /reject ${request._id} nếu từ chối)`
    );
...
// Lấy cấu hình rút tiền để hiển thị trên WebApp.
app.get('/api/withdraw-info', authMiddleware, asyncHandler(async (req, res) => {
  res.json({
    minVND: MIN_WITHDRAW_VND,
    maxVND: MAX_WITHDRAW_VND,
    goldPerVND: WITHDRAW_GOLD_PER_VND,
    balanceGold: req.player.gold,
    methods: [
...
// Người chơi tạo yêu cầu rút. Vàng được trừ ngay để tránh gửi nhiều yêu cầu cùng một số dư.
app.post('/api/request-withdraw', authMiddleware, asyncHandler(async (req, res) => {
  const { amountVND, method, accountName, accountNumber, bankName } = req.body || {};
  const amount = Number(amountVND);

  if (!Number.isInteger(amount) || amount < MIN_WITHDRAW_VND || amount > MAX_WITHDRAW_VND) {
    return res.status(400).json({
      error: `Số tiền rút phải từ ${MIN_WITHDRAW_VND.toLocaleString('vi-VN')}đ đến ${MAX_WITHDRAW_VND.toLocaleString('vi-VN')}đ.`,
    });
  }
  if (!['zalopay', 'bank'].includes(method)) {
    return res.status(400).json({ error: 'Phương thức rút không hợp lệ.' });
  }
  ...
  if (name.length < 2 || name.length > 80) return res.status(400).json({ error: 'Tên tài khoản không hợp lệ.' });
  if (number.length < 5 || number.length > 40) return res.status(400).json({ error: 'Số tài khoản/số điện thoại không hợp lệ.' });
  if (method === 'bank' && !bank) return res.status(400).json({ error: 'Vui lòng nhập tên ngân hàng.' });

  const goldAmount = amount * WITHDRAW_GOLD_PER_VND;
  const player = req.player;

  // Chỉ trừ vàng nếu số dư hiện tại đủ.
  const updated = await Player.findOneAndUpdate(
    { _id: player._id, gold: { $gte: goldAmount } },
    { $inc: { gold: -goldAmount } },
    { new: true }
  );
  if (!updated) return res.status(400).json({ error: `Bạn cần ${goldAmount.toLocaleString('vi-VN')} vàng để rút ${amount.toLocaleString('vi-VN')}đ.` });
  ...
  try {
    ...
    if (ADMIN_TELEGRAM_ID) {
      const methodName = method === 'zalopay' ? 'ZaloPay' : 'Ngân hàng';
      await sendTelegramMessage(ADMIN_TELEGRAM_ID,
        `💸 Yêu cầu RÚT TIỀN mới\n` +
        `Người chơi: ${player.username || 'ẩn danh'} (id: ${player.telegramId})\n` +
        `Số tiền: ${amount.toLocaleString('vi-VN')}đ\n` +
        `Vàng đã trừ: ${goldAmount.toLocaleString('vi-VN')}\n` +
        `Phương thức: ${methodName}\n` +
        `Tên: ${name}\n` +
        `Số tài khoản/SĐT: ${number}\n` +
        `${method === 'bank' ? `Ngân hàng: ${bank}\n` : ''}\n` +
        `Sau khi BẠN tự chuyển tiền thành công, gõ:\n` +
        `/withdraw_approve ${request._id}\n` +
        `Hoặc từ chối/hoàn vàng:\n` +
        `/withdraw_reject ${request._id}`
      );
    }
    ...
    return res.json({
      ...
      message: 'Đã gửi yêu cầu rút tiền. Admin sẽ kiểm tra và chuyển tiền thủ công.',
    });
  } catch (err) {
    // Nếu tạo request thất bại, hoàn lại vàng.
    await Player.updateOne({ _id: player._id }, { $inc: { gold: goldAmount } });
    throw err;
  }
}));

app.get('/api/my-withdrawals', authMiddleware, asyncHandler(async (req, res) => {
  const list = await WithdrawalRequest.find({ telegramId: req.player.telegramId })
    .sort({ createdAt: -1 }).limit(20).lean();
  res.json({ list });
}));
...
app.post('/api/telegram-webhook', asyncHandler(async (req, res) => {
  ...
  if (message && message.text && message.from) {
    ...
    if (isAdmin) {
      ...
      const withdrawApproveMatch = text.match(/^\/withdraw_approve\s+(\S+)/i);
      const withdrawRejectMatch = text.match(/^\/withdraw_reject\s+(\S+)/i);
      const withdrawListMatch = text.match(/^\/withdraw_list$/i);

      if (text === '/resetall') {
        await sendTelegramMessage(ADMIN_TELEGRAM_ID,
          '⚠️ Lệnh này sẽ XOÁ TOÀN BỘ tiến trình của MỌI người chơi — vàng về 50.000, cấp về 1, mất hết thú đang nuôi, mất hết boost/combo, reset cả PvP/điểm danh. KHÔNG THỂ HOÀN TÁC.\n\nGõ chính xác:\n/resetall confirm\nđể thực sự thực hiện.'
        );
      }
      ...
      if (text === '/resetall confirm') {
        await Player.updateMany({}, {
          $set: {
            gold: 50000, level: 1, xp: 0,
            owned: {}, eggs: {}, animalLevel: {}, lastCollected: {},
            speedBoost: {}, goldBoost: {}, fedUntil: {},
            comboFedUntil: 0, comboGoldBonusUntil: 0,
            claimedMilestones: [], weeklyGold: 0, weeklyGoldWeekStart: 0,
            checkinStreak: 0, lastCheckinAt: 0,
            pvpWins: 0, pvpLosses: 0, freeFoodTokens: 0,
          },
        });
        await sendTelegramMessage(ADMIN_TELEGRAM_ID, '✅ Đã reset TOÀN BỘ người chơi về trạng thái ban đầu (50.000 vàng, cấp 1, chưa nuôi con nào).');
      }

      // Chủ động làm mới bảng xếp hạng tuần ngay lập tức — KHÔNG đụng tới vàng/thú/cấp độ của
      // người chơi, chỉ đưa vàng-tuần (dùng để xếp hạng) về 0, coi như bắt đầu tuần mới sớm.
      if (text === '/resetweek') {
        const newWs = getCurrentWeekStart();
        await Player.updateMany({}, { $set: { weeklyGold: 0, weeklyGoldWeekStart: newWs } });
        await sendTelegramMessage(ADMIN_TELEGRAM_ID, '✅ Đã làm mới bảng xếp hạng tuần — vàng-tuần của mọi người chơi về 0. Vàng/thú/cấp độ KHÔNG bị ảnh hưởng.');
      }

      // Xem tổng số người chơi hiện tại + vài thống kê nhanh.
      if (text === '/players') {
        const ws = getCurrentWeekStart();
        const [total, paidCount, activeThisWeek] = await Promise.all([
          Player.countDocuments({}),
          Player.countDocuments({ hasEverPaid: true }),
          Player.countDocuments({ weeklyGoldWeekStart: ws, weeklyGold: { $gt: 0 } }),
        ]);
        await sendTelegramMessage(ADMIN_TELEGRAM_ID,
          `👥 Thống kê người chơi\n\n` +
          `Tổng số người chơi: ${total}\n` +
          `Đã từng nạp ít nhất 1 lần: ${paidCount}\n` +
          `Có bán trứng trong tuần này: ${activeThisWeek}`
        );
      }
      ...
      if (text === '/weeklytop') {
        ...
        const ranked = candidates
          ...
          .filter(p => p.weeklySpend >= MIN_WEEKLY_SPEND_TO_QUALIFY)
          .sort((a, b) => b.weeklyGold - a.weeklyGold)
          .slice(0, 10);
        ...
        if (!ranked.length) {
          ...
          const poolLabel = isFirstWeek ? `${pool.toLocaleString('vi-VN')}đ — CỐ ĐỊNH tuần đầu ra mắt` : `${pool.toLocaleString('vi-VN')}đ — 50% doanh thu ZaloPay tuần`;
          await sendTelegramMessage(ADMIN_TELEGRAM_ID, `🏆 Đua top tuần này (quỹ giải: ${poolLabel}):\n\n${lines.join('\n')}\n\nLiên hệ đúng id Telegram ở trên để chuyển thưởng qua ZaloPay/ngân hàng nhé.`);
        }
      }

      if (withdrawListMatch) {
        const pending = await WithdrawalRequest.find({ status: 'pending' })
          .sort({ createdAt: -1 }).limit(20).lean();
        if (!pending.length) {
          await sendTelegramMessage(ADMIN_TELEGRAM_ID, '💸 Hiện không có yêu cầu rút tiền đang chờ.');
        } else {
          const lines = pending.map(r =>
            `• ${r._id} — ${r.amountVND.toLocaleString('vi-VN')}đ — ${r.username || 'ẩn danh'} — ${r.method} — ${r.accountNumber}`
          );
          await sendTelegramMessage(ADMIN_TELEGRAM_ID, `💸 Yêu cầu rút đang chờ:\n\n${lines.join('\n')}`);
        }
      }

      if (withdrawApproveMatch || withdrawRejectMatch) {
        const requestId = (withdrawApproveMatch || withdrawRejectMatch)[1];
        const approve = Boolean(withdrawApproveMatch);
        let request = null;
        try { request = await WithdrawalRequest.findById(requestId); } catch { request = null; }

        if (!request) {
          await sendTelegramMessage(ADMIN_TELEGRAM_ID, `❌ Không tìm thấy yêu cầu rút: ${requestId}`);
        } else if (request.status !== 'pending') {
          await sendTelegramMessage(ADMIN_TELEGRAM_ID, `⚠️ Yêu cầu rút ${requestId} đã được xử lý (${request.status}).`);
        } else if (approve) {
          request.status = 'paid';
          request.processedAt = new Date();
          await request.save();
          await sendTelegramMessage(ADMIN_TELEGRAM_ID, `✅ Đã đánh dấu ĐÃ CHUYỂN tiền cho yêu cầu rút ${requestId}.`);
          await sendTelegramMessage(request.telegramId,
            `🎉 Yêu cầu rút ${request.amountVND.toLocaleString('vi-VN')}đ của bạn đã được admin đánh dấu đã chuyển thành công qua ${request.method === 'zalopay' ? 'ZaloPay' : 'ngân hàng'}.`
          );
        } else {
          request.status = 'rejected';
          request.processedAt = new Date();
          await request.save();
          await Player.updateOne({ telegramId: request.telegramId }, { $inc: { gold: request.goldAmount } });
          await sendTelegramMessage(ADMIN_TELEGRAM_ID, `❌ Đã từ chối yêu cầu rút ${requestId} và hoàn ${request.goldAmount.toLocaleString('vi-VN')} vàng.`);
          await sendTelegramMessage(request.telegramId,
            `❌ Yêu cầu rút ${request.amountVND.toLocaleString('vi-VN')}đ bị từ chối. ${request.goldAmount.toLocaleString('vi-VN')} vàng đã được hoàn lại vào tài khoản.`
          );
        }
      }
      ...
      if (approveMatch || rejectMatch) {
        ...
        if (!request) {
          ...
          const combo = COMBO_PACKAGES[request.foodType];
          const isFirstDeposit = player && !player.hasEverPaid;

          if (player && combo && combo.goldReward) {
            // Gói khởi đầu — cộng thẳng vàng, đánh dấu đã dùng (chỉ mua được 1 lần).
            player.gold += combo.goldReward;
            player.starterPackUsed = true;
          } else if (player && combo) {
            const expiry = Date.now() + combo.days * 24 * 60 * 60 * 1000;
            player.comboFedUntil = expiry;
            if (combo.goldBonusPercent > 0) player.comboGoldBonusUntil = expiry;
          } else if (player) {
          ...
          if (player) {
            if (isFirstDeposit) player.hasEverPaid = true; // mở khoá hoa hồng mời bạn 15%
            await player.save();
          }

          await sendTelegramMessage(ADMIN_TELEGRAM_ID, `✅ Đã duyệt yêu cầu ${requestId}.`);
          const itemName = combo ? combo.name : FOOD_TYPES[request.foodType].name;
          const scopeText = combo && combo.goldReward ? '' : combo ? 'cho TẤT CẢ con vật đang nuôi' : `cho con "${request.animalId}"`;
          const durationText = combo && combo.goldReward ? `+${combo.goldReward.toLocaleString('vi-VN')} vàng` : combo ? `${combo.days} ngày` : '24 giờ';
          const bonusText = isFirstDeposit ? `\n\n🎉 Chào mừng người ủng hộ mới! Hoa hồng mời bạn bè của bạn đã tăng lên 15%.` : '';
          await sendTelegramMessage(request.telegramId,
            `🎉 Yêu cầu mua ${itemName} ${scopeText} đã được duyệt! ${combo && combo.goldReward ? durationText : 'Hiệu lực ' + durationText}.${bonusText}`);
        } else {
          request.status = 'rejected';
          await request.save();
